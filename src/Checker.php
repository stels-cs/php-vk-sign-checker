...
class Checker {
  ...
  public static function checkVkAppsParams(array $params, string $secret): bool {
    $sign           = self::createVKAppsSign($params, $secret);
  ...
  public static function createVKAppsSign(array $params, string $secret): string {
    $signParams = [];
    ...
    return rtrim(strtr(base64_encode(hash_hmac('sha256', $signParamsQuery, $secret, true)), '+/', '-_'), '=');
  }
   ...
   * @param string $secret
   * @return bool
   * @deprecated Этот метод использовался для проверки подписи приложений сообщества, эта платформа уже все
   */
